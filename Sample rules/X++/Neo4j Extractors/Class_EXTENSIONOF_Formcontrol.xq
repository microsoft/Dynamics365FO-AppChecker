...
(: Export formcontrol extension information in CSV format :)
...
let $extensions := <Extensions>
{
    ...
    where lower-case($attr/AttributeExpression/IntrinsicAttributeLiteral/@FunctionName) = 'formcontrolstr'
    let $extensionof := lower-case($attr/AttributeExpression/IntrinsicAttributeLiteral/@Arg1)
    let $control := lower-case($attr/AttributeExpression/IntrinsicAttributeLiteral/@Arg2)
    
    return <Extension ExtendedType='{$extensionof}' Control='{$control}' ExtensionClass='{lower-case($c/@Name)}' ExtensionClassPackage='{lower-case($c/@Package)}'>
    </Extension>
}</Extensions>
...
let $r := <ExtensionsOnMethods>
{
  ...
  return <Record>
      ...
      <Extended name=':END_ID'>{ lower-case("/" || map:get($forms, $e/@ExtendedType) || "/forms/" || $e/@ExtendedType || "/controls/" || $e/@Control )}</Extended>
