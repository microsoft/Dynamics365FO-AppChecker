...
(: Export interesting metrics for tables. The metrics provided
...
let $r := <ClassMetrics>
{
    ...
    return <Record>
        <Artifact name="Artifact:ID">{lower-case("/" || $a/@Package || "/tables/" || $a/@Name)}</Artifact>
        <Package name='Package'>{lower-case($a/@Package)}</Package>
        <Name name="Name">{lower-case($a/@Name)}</Name>
        <Kind name="Kind">Table</Kind>
