...
let $extensions := <Extensions>
{
    ...
    where lower-case($attr/AttributeExpression/IntrinsicAttributeLiteral/@FunctionName) = 'formdatasourcestr'
    ...
    return <Extension ExtendedType='{$extensionof}' Datasource='{$datasource}' ExtensionClass='{lower-case($c/@Name)}' ExtensionClassPackage='{lower-case($c/@Package)}'>
    </Extension>
}</Extensions>
...
let $r := <ExtensionsOnMethods>
{
  ...
  return <Record>
      ...
      <Extended name=':END_ID'>{ lower-case("/" || map:get($forms, $e/@ExtendedType) || "/forms/" || $e/@ExtendedType || "/datasources/" || $e/@Datasource )}</Extended>
