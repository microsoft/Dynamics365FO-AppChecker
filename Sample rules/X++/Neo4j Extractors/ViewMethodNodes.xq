...
(: Extract view method information in CSV format :)
...
let $r := <MethodsOnViews>
{
    ...
    return <Record>
        <Artifact name='Artifact:ID'>{lower-case("/" || $a/@Package || "/views/" || $a/@Name || "/methods/" || $m/@Name) }</Artifact>
        <Package name='Package'>{lower-case($a/@Package)}</Package>
        <View name='View'>{lower-case($a/@Name)}</View>
