...
let $r := <MethodsOnFormDataFields>
{
    ...
    for $datafield in $datasource/FormDataField
    for $m in $datafield/Method
    ...
    let $name := $a/@Name
    return <Record>
        <Artifact name="Artifact:ID">{lower-case("/" || $a/@Package || "/forms/" || $name || "/datasources/" || $datasource/@Name || "/datafields/" || $datafield/@Name ||  "/methods/" || $m/@Name) }</Artifact>
