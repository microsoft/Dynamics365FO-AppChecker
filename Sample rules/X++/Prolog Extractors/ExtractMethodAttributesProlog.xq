...
(:~ Extract information about attributes applied on classes as prolog clauses :)
...
let $result := (
    ...
    return "methodattribute(c('" || lower-case($a/@Name) || "'), m('" || lower-case($m/@Name) || "'), " || local:RenderAttribute($attr) || ").",
  
    for $a in /Table[lower-case(@Package)=$packages]
    for $m in $a/Method
    for $attr in $m/AttributeList/Attribute
    return "methodattribute(t('" || lower-case($a/@Name) || "'), m('" || lower-case($m/@Name) || "'), " || local:RenderAttribute($attr) || ").",
  
    for $a in /Form[lower-case(@Package)=$packages]
    for $m in $a/Class/Method
    for $attr in $m/AttributeList/Attribute
    return "methodattribute(f('" || lower-case($a/@Name) || "'), m('" || lower-case($m/@Name) || "'), " || local:RenderAttribute($attr) || ").",
  
    for $a in /Query[lower-case(@Package)=$packages]
    for $m in $a/Class/Method
    for $attr in $m/AttributeList/Attribute
    return "methodattribute(q('" || lower-case($a/@Name) || "'), m('" || lower-case($m/@Name) || "'), " || local:RenderAttribute($attr) || ")."
)
